...
Each file is named `{npub}.json` and contains the member record. The [`members.json`](https://github.com/lonniev/dpyc-community/blob/main/members.json) index is auto-generated by CI from these individual files and serves as the read-only source of truth for downstream consumers. When a Tollbooth Authority certifies a purchase, it checks this registry to verify the operator's `npub` is active.
...
4. **PR reviewed and merged** — the CI workflow validates individual member files and auto-regenerates `members.json`.
