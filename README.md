...
**The registry** is stored as individual JSON files under [`members/`](https://github.com/lonniev/dpyc-community/tree/main/members), organized by role:
...
Each file is named `{npub}.json` and contains the member record. The [`read-only-lookup-cache.json`](https://github.com/lonniev/dpyc-community/blob/main/members/read-only-lookup-cache.json) index is a generated artifact auto-rebuilt by CI from these individual files. Downstream consumers (like the DPYC Oracle) use this cache for fast member lookup. When a Tollbooth Authority certifies a purchase, it checks this registry to verify the operator's `npub` is active.
