...
**Self-similar pattern**: An Authority is itself an Operator at a higher level. It purchases certification credits from its upstream Authority, then sells certified purchase orders to its downstream Operators. The same Tollbooth SDK serves both roles — the only difference is configuration. Since v0.5.0, the Authority service runs directly on `OperatorRuntime` (with `purchase_mode="auto"`, which derives its certify-up behavior from its position in the registry chain), making the self-similar pattern literal rather than conceptual: an Authority uses the same runtime, ledger, and constraint engine as every other Operator. Each non-Prime Authority holds a **pre-funded balance at its upstream Authority** — its certification capacity — which it draws down as it certifies downstream purchase orders and replenishes through its own `purchase_credits` call, exactly as any Operator funds its balance. Certification is satisfied against that pre-funded balance; there is no real-time upstream cascade.

**Certification flow**: When an Operator needs to fund customer credits, it requests a certified purchase order from its sponsoring Authority. The Authority debits its ad-valorem certification fee (default 2%) from the Operator's pre-funded balance, signs a Nostr event certificate (kind 30079, Schnorr/BIP-340), and returns it. The Operator presents that certificate to the Tollbooth SDK, which verifies the signature and credits the customer's ledger. A non-Prime Authority maintains its own pre-funded balance one tier up and replenishes it the same way — so certification fees compound across the chain, but each is collected when credits are purchased, never during a customer's tool call.
